...
const translateFile = async (inputPath, outputPath) => {
  try {
    const languages = ['de', 'es', 'pt', 'fr'];

    const jsonFile = await readFile(inputPath);
    ...
    languages.forEach(async (language) => {
      ...
      await writeFile(`${outputPath}/${language}/translation.json`, JSON.stringify(translatedContent, null, 2));
    });
  } catch(error) {
    throw error;
  }
...
translateFile(resolve(__dirname, './locales/en/translation.json'), resolve(__dirname, './locales'));
